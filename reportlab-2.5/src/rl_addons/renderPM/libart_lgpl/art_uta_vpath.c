		  ...
		  int *rbuf, int rbuf_rowstride)
{
  ...
  xmin = (int)floor (MIN(x0, x1));
  xmax = MAX(x0, x1);
  xmaxf = (int)floor (xmax);
  xmaxc = (int)ceil (xmax);
  ymin = (int)floor (MIN(y0, y1));
  ymax = MAX(y0, y1);
  ymaxf = (int)floor (ymax);
  ymaxc = (int)ceil (ymax);
  ...
  if (xt0 == xt1 && yt0 == yt1)
	...
	bb = ART_UTA_BBOX_CONS(MIN(ART_UTA_BBOX_X0(bb), (unsigned)xf0),
			   MIN(ART_UTA_BBOX_Y0(bb), (unsigned)yf0),
			   MAX(ART_UTA_BBOX_X1(bb), (unsigned)xf1),
			   MAX(ART_UTA_BBOX_Y1(bb), (unsigned)yf1));
      uta->utiles[ix] = bb;
    }
  else
    {
